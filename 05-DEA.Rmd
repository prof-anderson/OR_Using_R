...
# library(bookdown); library(rmarkdown); rmarkdown::render("05-DEA.Rmd", "pdf_book")
```
...
```{r first_dataset_table, fig.margin=FALSE}
# grid.table(temp, caption="First Dataset for DEA")
pander(temp, caption="First Dataset for DEA")
#kable(temp, caption="First Dataset for DEA", 
#      format="latex")
```

The above commands create matrices that hold the data and have named rows and columns to match. The `<-` symbol is a key function in R and means to assign what is in the right to the object on the left. \vspace{12pt}

For benchmarking, we want to know which ones are doing the best job. \vspace{12pt}

Can you tell which stores represent the best tradeoff between inputs and outputs? None of the stores are strictly dominated by any of the other stores. Dominance would be producing more outputs using less input so let's move on to looking at it graphically. \vspace{12pt}
...
```{r ompr_mult_IO, message=FALSE, results='asis'}
...
colnames(results.lambda) <- c("$\\lambda_A$", "$\\lambda_B$",
                              "$\\lambda_C$", "$\\lambda_D$")
pander(cbind(results.efficiency, results.lambda),  caption=
...
kable (cbind(results.efficiency, results.lambda),
...
```{r, echo=FALSE}
kable(cbind(results.efficiency, poscol(results.lambda)), 
       caption=
...
```{r DEA_VRS_Frontier_Plot, echo=FALSE, fig.margin=TRUE, fig.height=4, fig.cap="BCC (VRS) Efficiency Frontier"}
library(Benchmarking, quietly=TRUE)
...
```{r DEA_IRS_Frontier_Plot, echo=FALSE, fig.margin=TRUE, fig.height=4, fig.cap="Increasing (Non-Decreasing) Returns to Scale Efficiency Frontier"}
library(Benchmarking, quietly=TRUE)
...
```{r DEA_DRS_Frontier_Plot, echo=FALSE, fig.margin=TRUE, fig.height=4, fig.cap="Decreasing  (Non-Increasing) Returns to Scale Efficiency Frontier"}
library(Benchmarking, quietly=TRUE)
...
kable(cbind(eff.crs, eff.vrs, eff.drs, eff.irs), 
...
kable(cbind(UTT_CCR.Res[,1, drop=FALSE],
             UTT_BCC.Res), digits=6,
       caption=
...
-   The use of `drop=FALSE` in the subsetting of UTT_CCR.Res enables retaining the original structure of the matrix, which allows the column name to remain.  The default is drop=TRUE which simplifies the data object into a form that doesn't have a column name. \vspace{12pt}
...
kable(c(res.vweight[k,],res.uweight[k,]),
...
kable (cbind(res.efficiency, res.vweight, 
              res.uweight),digits=6, 
...
#pander(head(cbind(res.efficiency,poscol(res.lambda[1:25,]))), 
#       round=4, caption="Selected Input-oriented Efficiency Results with Slack Maximization")
kable(head(cbind(res.efficiency,poscol(res.lambda[1:25,]))),
      caption="Selected Input-oriented Efficiency Results with Slack Maximization",
