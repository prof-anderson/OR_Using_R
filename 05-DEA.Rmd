...
Figure <- DrawIOdiagram (c("Employees (FTE)" ), 
                         c("Sales ($/week)"), 
                         '"\nStore\n "')
...
A key way to begin the mathematical development of the envelopment model is to ask, can you find a combination of units that produces a target with at least as much output using less input? The blend of other units is described by a vector $\lambda$.  Another way to denote this is $\lambda_j$ is the specific amount of a unit _j_ used in setting the target for performance for the unit being studied, _k_.  In this case, _k=B_ since we are analyzing store B.  Furthermore, we can state that we want to find how much input our target for B needs compared what B actually used. This value for input usage is a unitless variable, $\theta$ where $\theta =1$ indicates that a reduction cannot occur for each input while maintaing B's level of output.  Similarly, a value of $\theta =0.9$ indicates a 10% reduction in input usage could be obtained.     
\vspace{12pt}
...
To solve for other stores, we would simply change the values on the right hand side of the constraints and solve. This means that we solve as many linear programs as there are units to evaluate.  We are in effect iterating for _k=1,...,Number of Units_.
...
This gives us the following linear program, often abbreviated as an LP.
