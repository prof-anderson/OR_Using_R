...
kbl(results_B, digits=4,booktabs=T, escape=F, 
  ...
  kable_styling(latex_options = "hold_position") |>
  footnote(general="Creating a table with LaTeX in column names is covered in Appendix C.")
```
...
kbl (cbind(res.efficiency, poscol(res.lambda)),
     ...
     col.names = c("$\\theta^{CRS}$", "$\\lambda_{text{NYU}}$",
                   "$\\lambda_{text{UW-Madison}}$", 
                   "$\\lambda_{text{Cal Tech}}$"),
...
These results are for just the last unit examined.  Care must be had in interpreting them \
