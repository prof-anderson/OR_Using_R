...
Next, let's try running this using a powerful commercial solver.  \vspace{12pt}

```{r solve_using_gurobi, echo=FALSE, eval=FALSE}
library(ROI.plugin.gurobi)
res_Mail3 <- solve_model(Mail2, 
                        with_ROI(solver = "gurobi", 
                                 MIPGap=0.1))
    # Solve with a 10% MIP suboptimality tolerance factor (gap)
assigned3 <- res_Mail3 %>% 
 get_solution(Vx[s,w]) %>%
 filter(value >.9) %>%
 select (s,w)
rownames(assigned3)<-c(names(Customers[assigned3[,1]]))

assigned3a <- tibble::rownames_to_column(assigned3)
table_results3<-""
weeksmail3a<-""
for (week_counter in 1:Weeks) {
  weeksmail3a<- assigned3a %>%
  filter(w==week_counter) %>%
  select(rowname)
colnames(weeksmail3a)<-paste0("Week ",week_counter)
table_results3 <- append(table_results3,weeksmail3a)
}
pander (table_results3[-1],   # Removes first item
       caption="Results from Gurobi")
```
