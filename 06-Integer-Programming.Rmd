...
#library(bookdown); library(rmarkdown); library(tufte); rmarkdown::render("06-Integer-Programming.Rmd", "tufte_book2")
...
```{r IncAssemHrs_ch6, fig.margin=FALSE}
...
```{r, fig.margin=TRUE}
kbl (temp1, booktabs=T, caption="Production plans with an additional assembly hour based on variable type.")
```
...
```{r Fig-LPRelaxation, echo=FALSE, fig.margin=TRUE, out.width="2in"}
...
```{r Fig-Ex_Inline_code_chunk, echo=FALSE, fig.margin=TRUE, out.width="2in"}
...
kbl (LPSubIII_res,
       booktabs=T, caption="Acme's Optimal Production Plan 
...
**Exercise 6.1** (Create and solve the model)
...
**Exercise 6.2** (No Project is Repeated)
...
**Exercise 6.3** (No Partial Projects)
...
**Exercise 6.4** (Interconnects Need Something to Connect)
...
**Exercise 6.6** (Solving Each Case)
...
**Exercise 6.8** (Redesign Widget 3)
...
**Exercise 6.9** (Reducing Fixed Costs)
