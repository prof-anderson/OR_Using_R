...
```{marginfigure}
Note that our Acme example only specified digits of accuracy for resource usage and may have only digit of accuracy for profitability per product.
```
...
I <- matrix(c(12,24,20,8, 16),ncol=1,dimnames=list(LETTERS[1:5],"Inv"))
N <- matrix(c(2.0, 3.6, 3.2, 1.6, 2.8), ncol=1,dimnames=list(LETTERS[1:5],"NPV"))
pander (cbind(I,N), caption="Data for the Outtel In-Class Exercise")
```
