...
Since $M$ serves to impose an upper bound on $W_1$, this might suggest how we can use this information to pick an appropriate value for M and that we can do so for each product separately, using a separate value $M_i$ for each widget.  Let's examine the constraints for the production plan in the situation gives all resources to the production of Widget 1 and nothing to Widget 2 and Widget 3.  
...
The resulting Big M constraints are then following.

$$W_1 \leq 500Y_1, \\W_2 \leq 400Y_2, \\W_3 \leq 250Y_3.$$

We can rewrite this into standard form by collecting all terms with variables on the left hand side.  Of course, `ompr` can do this manual work for us so it is not necessary.  
