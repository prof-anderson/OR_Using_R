...
library (png)
library (gridExtra)
# library(bookdown); library(rmarkdown); rmarkdown::render("06-Integer-Programming.Rmd", "tufte_book2")
```
...
fc_base_model <- MIPModel() %>%
 ...
 set_objective(15*Vw1 + 10*Vw2 + 25*Vw3 
               ...
               "max") %>%
 add_constraint(2*Vw1 + 1*Vw2 + 4*Vw3 <= 1000) %>% 
...
colnames(fc_M_summary)<-c("Obj Func Value", "$w_1$","$w_2$","$w_3$",
                             "$y_1$","$y_2$","$y_3$")
