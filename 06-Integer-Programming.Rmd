...
rmarkdown::render("06-Integer-Programming.Rmd", "pdf_book")
# rmarkdown::render("06-Integer-Programming.Rmd", "ioslides_presentation")
# rmarkdown::render("06-Integer-Programming.Rmd", "beamer_presentation")
```


```{r, eval=FALSE, include=FALSE}
# This is an attempt to create a revealjs version of the book.
library(rmarkdown); library(bookdown); library(revealjs)
rmarkdown::render("06-Integer-Programming.Rmd", revealjs_presentation(
  incremental = FALSE, center = FALSE,
  slide_level = 4, smart = FALSE,
...
## Example of Major Integrality Impact
...
colnames(acme_res_lp) <- list("$x_1$", "$x_2$", "Profit")
colnames(acme_res_ip) <- list("$x_1$", "$x_2$", "Profit")
rownames(acme_res_ip) <- "IP Solution"

kbl (rbind(acme_res_lp,acme_res_ip), booktabs=T, digits=3, escape=F,
...
```{r SolveLPRelaxation}
colnames(acme_res_lp) <- list("$x_1$", "$x_2$", "Profit")
kbl (acme_res_lp, booktabs=T, digits=3, escape=F,
...
colnames(LPSubI_res) <- list("$x_1$", "$x_2$", "Profit")

```

```{r, echo=FALSE}
# rownames(LPSubI_res) <- ""
kbl (LPSubI_res, booktabs=T, digits=3, row.names=F, escape=F,
...
colnames(LPSubIII_res) <- list("$x_1$", "$x_2$", "Profit")

kbl (LPSubIII_res, booktabs=T, row.names=F, escape=F,
...
colnames(LPSubIV_res) <- list("$x_1$", "$x_2$", "Profit")

kbl (LPSubIV_res, booktabs=T, digits=3, row.names=F, escape=F,
     caption=
...
colnames(LPSubV_res) <- list("$x_1$", "$x_2$", "Profit")

rownames(LPSubV_res) <- ""

kbl (LPSubV_res, booktabs=T, digits=3, escape=F,
     caption=
...
colnames(LPSubVI_res) <- list("$x_1$", "$x_2$", "Profit")
```

```{r, echo=FALSE}
rownames(LPSubVI_res) <- ""
kbl (LPSubVI_res, booktabs=T, digits=3, escape=F,
     caption=
"Infeasible (Impossible) Production Plan based on Subproblem VI") |>
  kable_styling(latex_options = "hold_position")
```
...
colnames(LPSubVII_res) <- list("$x_1$", "$x_2$", "Profit")
rownames(LPSubVII_res) <- ""
kbl (LPSubVII_res, booktabs=T, digits=3, escape=F,
     caption=
...
colnames(LPSubVIII_res) <- list("$x_1$", "$x_2$", "Profit")
kbl (LPSubVIII_res, booktabs=T, digits=3, escape=F,
     caption=
...
colnames(LPSubII_res) <- list("$x_1$", "$x_2$", "Profit")
rownames(LPSubII_res) <- ""

kbl (LPSubII_res, booktabs=T, digits=3, escape=F,
...
rownames(BandB) <- list("LP Relaxation",  "Subproblem I", 
                        ...
                        "Subproblem II")
colnames(BandB) <- list("$x_1$", "$x_2$", "Profit")

kbl (BandB, booktabs=T, escape=F,
...
$$
 \begin{split}
 \begin{aligned}
    \text{Max    } & 15\cdot w_1+10\cdot w_2+25\cdot w_3 - 1000\cdot y_1 - 1500\cdot y_2 - 2000\cdot y_3 \\
    \text{s.t.:  } & 2\cdot w_1+1\cdot w_2+3\cdot w_3 \leq 1000  & \text{[Pre-Processing]}\\
                   & 1\cdot w_1+5\cdot w_2+2\cdot w_3 \leq 2000  & \text{[Machining]}\\
                   & 2\cdot w_1+1\cdot w_2+1\cdot w_3 \leq 1000  & \text{[Assembly]}\\
                   & 3\cdot w_1+2\cdot w_2+1\cdot w_3 \leq 1500  & \text{[Quality Assurance]}\\
                   & w_1 \leq 500  & \text{[Capacity Widget 1]}\\
                   & w_2 \leq 1000 & \text{[Capacity Widget 2]}\\
                   & w_3 \leq 750  & \text{[Capacity Widget 3]}\\
                   & w_1, \; w_2, \; w_3 \geq 0   & \text{[Non-Negativity]}\\
                   & y_1, \; y_2, \; y_3 \; \in \{0,1\}   & \text{[Binary]}\\
  \end{aligned}
  \end{split}
  (\#eq:FixedChargeBase)
$$
...
$$
 \begin{split}
 \begin{aligned}
      2 \cdot w_1+1 \cdot 0+3 \cdot 0 & \leq 1000 & \text{[Pre-Processing]}\\
      1 \cdot w_1+5 \cdot 0+2 \cdot 0 & \leq 2000 & \text{[Machining]}\\
      2 \cdot w_1+1 \cdot 0+1 \cdot 0 & \leq 1000 & \text{[Assembly]}\\
      3 \cdot w_1+2 \cdot 0+1 \cdot 0 & \leq 1500 & \text{[Quality Assurance]}\\
      w_1 & \leq 500 & \text{[Capacity Widget 1]}\\
      0 & \leq 1000 & \text{[Capacity Widget 2]}\\
      0 & \leq 750 & \text{[Capacity Widget 3]}\\
  \end{aligned}
  \end{split}
...
$$
 \begin{split}
 \begin{aligned}
      2 \cdot w_1 & \leq 1000 & \text{[Pre-Processing]}\\
      1 \cdot w_1 & \leq 2000 & \text{[Machining]}\\
      2 \cdot w_1 & \leq 1000 & \text{[Assembly]}\\
      3 \cdot w_1 & \leq 1500 & \text{[Quality Assurance]}\\
      w_1 & \leq 500 & \text{[Capacity Widget 1]}\\
  \end{aligned}
  \end{split}
...
$$
 \begin{split}
 \begin{aligned}
      w_1 & \leq \frac{1000}{2} = 500 & \text{[Pre-Processing]}\\
      w_1 & \leq 2000 & \text{[Machining]}\\
      w_1 & \leq \frac{1000}{2} = 500 & \text{[Assembly]}\\
      w_1 & \leq \frac{1500}{3} = 500 & \text{[Quality Assurance]}\\
      w_1 & \leq 500 & \text{[Capacity Widget 1]}\\
  \end{aligned}
  \end{split}
...
$$
 ...
 \begin{aligned}
    ...
    \text{s.t.:  } & 2w_1+1w_2+3w_3 \leq 1000 & \text{[Pre-Processing]}\\
                   & 1w_1+5w_2+2w_3 \leq 2000 & \text{[Machining]}\\
                   & 2w_1+1w_2+1w_3 \leq 1000 & \text{[Assembly]}\\
                   & 3w_1+2w_2+1w_3 \leq 1500 & \text{[Quality Assurance]}\\
                   & w_1 \leq 500 & \text{[Capacity Widget 1]}\\
                   & w_2 \leq 1000 & \text{[Capacity Widget 2]}\\
                   & w_3 \leq 750 & \text{[Capacity Widget 3]}\\
                   & \textcolor{red}{w_1 - 500y_1 \leq 0} & \text{[Widget 1 Linking]}\\
                   & \textcolor{red}{w_2 - 400y_2 \leq 0} & \text{[Widget 2 Linking]}\\
                   & \textcolor{red}{w_3 - 334y_3 \leq 0} & \text{[Widget 3 Linking]}\\
                   & w_1, w_2, w_3 \geq 0 \\
                   & y_1, y_2, y_3 \in \{0,1\}  \\
  \end{aligned}
  \end{split}
