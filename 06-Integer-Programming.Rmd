...
$$
 ...
 \begin{aligned}
  ...
  \end{split}
$$
At first glance, this looks the same as our earlier drone eample but the last constraint cannot be solved directly strictly using linear programming. It is instead referred to as an integer linear program or ILP.\
...
rbind(LPSolR$solution, 
           IPSolR$solution)

```

```{r Fig-LP-Feasible-Region, echo=FALSE, out.width="45%", fig.show='hold', fig.align='center', fig.cap="LP Region and 10 IP Feasible Solutions" }
knitr::include_graphics( c ("images/BB01.PNG", 
                            "images/BB03.PNG"))
```
...
rownames(LPSubI_res) <- ""
kbl (LPSubI_res, booktabs=T, digits=3,
...
```{r Fig-Subproblem-III-and-II, echo=FALSE, out.width="3in", fig.align='center',  fig.show='hold', fig.cap="Subproblem III-An IP Feasible Solution"}
...
kbl (LPSubIV_res, booktabs=T, digits=3,
     caption=
...
At first glance, this looks okay but if you look more closely at the value of $x_2$, and the surfactant constraint, $x_1+3x_2=9\not\leq8.25$, there isn't enough enough surfactant to make three units of the second product. This means that by inspection we can see that this production plan is infeasible! What is going on here? We should check the status of the solver before examining the result.
...
The ompr's status value of this solved LP indicates that subproblem IV is *infeasible.* It still returned values that were used when it determined that the problem was infeasible which is why it gave the results in the previous table. From here on out, it is a good reminder to check the status. Note that it can be used as an in-line evaluated expression to simply say was it feasible or not? Yes, it was `r LPSubIV$status`. 
...
```{r Fig-Subproblem-VI, echo=FALSE, out.width="45%", fig.show='hold', fig.align='center', fig.cap="Subproblem VI"}
...
```{r SolveLPSubVIII}
LPSubVIII <- add_variable(LPRmod, Vx1, type = "continuous", 
                        lb = 1,) 
LPSubVIII <- add_variable(LPSubVIII, Vx2, type = "continuous",
                        lb = 4) 
