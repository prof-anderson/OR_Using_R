...
The `runif` function generates a uniform random number between 0.0 and 1.0 such as perhaps 0.32721.
This would be multiplied by our grid size multiplier of 1,000 resulting in 327.21.
...
```{r Plot_Map_of_Customers_and_Warehouses, fig.cap="Map of Customer and Warehouse Locations"}
...
```{r plot_warehouses, echo=TRUE, fig.fullwidth=TRUE, fig.cap="Optional Warehouse Locations Customer Assignments"}
p + 
...
We'll take the results and combine them with other shortly.
...
A code chunk is provided without evaluation in order to avoid issues of reproducibility.
...
```{r lp_package_download_stats,  echo=FALSE, fig.width=10, fig.height=6, fig.fullwidth=TRUE, fig.cap="Monthly Downloads of Optimzation Packages"}
...
```{r ROI_Package_Stats, echo=FALSE, fig.width=10, fig.height=6, fig.fullwidth=TRUE, fig.cap="Monthly Downloads of Major ROI LP Plugins"}
...
```{r package_download_stats, fig.width=10, fig.height=6, fig.cap="Monthly Downloads of ompr"}
...
In other words, any feasible solution would be acceptable. Some people would characterize this type of problem as a constraint satisfaction problem rather than optimization but our tools of optimization are still useful.
...
beg_invA <- 71  # Inv at beginning of week 1
beg_invB <- 98
...
\begin{aligned}
\text{min   } \; & \sum\limits_{t=1}^{4} (C^P_A\cdot x_{t,A} + C^S_A\cdot y_{t,A} + C^I_A\cdot z_{t,A}+\\
\text{s.t.: } \; & C^P_B\cdot x_{t,B} + C^S_B\cdot y_{t,B} + C^I_B\cdot z_{t,B})\\
                 & z_{1,A}=B_{A} + x_{1,A} - D_{1,A}\\
...
```{r, eval=TRUE}
promod <- add_constraint (promod, beg_invA + VxA[1] - demA [1] == VzA[1] )

promod <- add_constraint (promod, beg_invB + VxB[1] - demB [1] == VzB[1] )
...
var_list <- c("$x_{t_A}$", "$x_{t_B}$", "$y_{t_A}$", "$y_{t_B}$",
              "$z_{t_A}$", "$z_{t_B}$") 
Solution <- cbind(var_list, Solution)
colnames(Solution) <- c("Variable", "Week 1", "Week 2", "Week 3", "Week 4")  
rownames(Solution) <- c("Ant Production", "Bat Production",
                        "Ant Setup", "Bat Setup",
                        "Ant Inventory", "Bat Inventory")  
kbl (Solution, booktabs=T, escape=F,
...
Similar to Warehouse problem above, here we have Chemical Factories for which 
we need to decide the locations. We need to set up 2 factories with 100 residents 
in the town. The town government is trying to keep the factories at maximum 
possible distance from the residents to avoid any decrease the impacts and risk of mishaps.

Taking the randomly generated setup costs for factories similar to warehouses,
run the analysis to decide the location for these 2 chemical factories which are 
