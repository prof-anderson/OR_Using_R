...
```{r setup_chapter7, echo=FALSE, warning=FALSE}
...
Similarly, the variable $x_{i,j}$ is the decision of assigning customer $i$ to warehouse $j$.  It also needs to be a binary variable since partial assignments are not allowed.  Therefore, $x_{3,7}=1$ means that customer _3_ is assigned to warehouse _j_ and we would expect $x_{3,8}=0$ since a customer can't be assigned to two warehouses.
...
#  add_constraint(sum_expr(Vx[i, j], i = 1:n) <= 4, j = 1:m) %>% 
...
The number of _x_ (or `Vx`) variables is $m \cdot n =20 \cdot 100$ or 2000 and the number of _y_ (or `Vy`) variables is $m$ or 20, the total number of variables is 2020 which matches the model summary. A similar calculation can be done on the constraints.  The number of variables and constraints make this a non-trivial sized MIP problem. Fortunately, solving still turns out to be pretty easy on a personal computer but be cautious of solving this on a cloud instance.
\vspace{12pt}
...
At this point, we are really interested in the small minority of non-zero variables.  Only 5% of the _x_ variables are not zero. 
\vspace{12pt}
...
4. Install gurobi's R package.  Note that the package file is not in CRAN and will need to be installed directly from the gurobi installed files on your local computer.  (For example in my windows install, it it was in `C:\\gurobi900\win64\R` directory.)
...
The respective `ROI.plugin` for each solver does not in general accept all parameters that a solver can use.  This may mean some trial and error is involved.  The [Gurobi documentation](https://www.gurobi.com/documentation/9.0/refman/parameters.html#sec:Parameters) provides a comprehensive list of parameters.  I've tested and the following commonly used parameters appear to work.
...
timeresults <- rbind(glpktime1, symphonytime1, 
                     symphonytimeff, lpsolvetime1, gurobitime1)
