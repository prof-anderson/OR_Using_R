---
title: "07-More Mixed Integer Models"
author: "Tim Anderson"
date: "2/9/2019"
output:
...
Next we set the number of customers to be 100.  
...
The runif function generates a uniform random number between 0.0 and 1.0 such as perhaps 0.327216. This would be multiplied by our grid size multipler resulting in 327.216.  This is then rounded to 327.  
...
This is a non-trivial sized MIP problem. Since the number of _x_ (or `Vx`) variables is $m \cdot s$ or 2000 and and the number of _y_ (or `Vy`) variables is $m$ or 20, the total number of variables is 2020 which matches the model summary.
...
At this point, we are really interested in the small minority of non-zero variables.  Only 5% of the _x_ variables are not zero.  _Challenge: Do you know why it must be 5% in this problem?)_ We can do some processing to extract the non-zero _x_ (or `Vx`) variables from the zero values.
...
The first line wraps the loading of the library in a function that strips out a lot of messages upon loading that we don't need to worry about for now.  I need to remember to use `suppressPackageStartupMessages` more often!  

The next command matching `matching <- result %>%` does a few things.  It using the piping operator `%>%` to pass the `result` object into being used by the following command and then taking everything that gets done in this piped sequence will be assigned to `matching`.   

The command `get_solution(Vx[i,j]) %>%` extract solution values for the x variables (`Vx`) from the solved model object `result` piped in from the previous stage.  This will give us a data object consisting of 2000 variables values, 95% of them being zero.  

The command `filter(value > .9) %>%` takes the previous command's 2000 variable values and only keeps the ones for which the value is larger than 0.9. Since these are binary variables, they should all be exactly 1.0 but just in case there is any numerical anomaly where a value is passed as a value close to one, 0.99999999999, it is best to test with some tolerance.  This command then only passes to the next command the 100 non-zero values.

The last command in this code chunk `select(i, j)` says to only select the columns named `i` and `j`  to pass out.  Notice that does not pass the actual value (we know it is one!).  Also, note that it does not including piping operator `%>%` which means that it is the end of this piped sequence.  

Let's review what the two versions of the results look like.  

```{r}
pander(head( get_solution(result, Vx[i,j])), 
       caption = "Example of some of the raw results for variable x")

pander(head( matching), 
       caption = "Example of some of the processed results for variable x")
```

Notice that in the table, among the first six entries (obtained using the `head` function, we didn't get any customers assigned to a warehouse.  This isn't surprising given that only one in twenty values are non-zero.  

The second table shows the `matching` object that simply lists each pairing of customer to warehouse that is used.  

This clean and simple listing is interesting and will then be used for adding lines to the earlier `ggplot` model.
...
Notice that this gives the beginning and ending coordinates lines to show the connections between customers and warehouses.
...
We can see that warehouse 17 has the most customers.  
...
The `plot_warehouses` again uses `dplyr` functions to create a summary of the important information about the warehouses used:  where they are located, costs, and number of customers served.
...
* Demonstrate a large, classic, and common industrial application of optimization.
* Demonstrate creation of simulated data
* Demonstrate how to handle double subscripted variables 
* Show how to do timing of R functions
* Explore solving with multiple optimization routines.
* Visualize results graphically
* Process results using `dplyr`
...
* The model can also be extended to allow for additional application characteristics such as warehouse capacity and customer demand.
* Using actual cities in a region for locations rather than randomly generated data
* Examining the solving time of larger problems
* Testing other solvers (ex. does Gurobi run faster?)
* Testing the impact of passing modeling parameters

The same concepts can also be applied to other applications.  

## Solving MIPs with Different Solvers

Let's revisit the Warehouse assignment problem.  The emphasis in this section is not model building but comparing the use of different solvers.  Since we have the model already defined, we can easily pass it to the different solvers for optimizing.  There is a large collection of other solvers that can be used with R.[^1]

[^1]: This section is not covered in Dirk's article.

Let's use the `tictoc` package for examining computation time.  

We'll demonstrate the performance for illustration purposes.  Again, do not run the following code chunks in this section on a cloud-based service unless you are certain that imposing a non-trivial computational load is acceptable. 
...
The `glpk` solver worked.  Let's move on to testing the `symphony` solver.
...
$$\begin {split}
\begin {aligned}
x_{1,1,k}+x_{1,2,k}+x_{1,1,3}+x_{2,1,k}+x_{2,2,k}+x_{2,1,3}+x_{3,1,k}+x_{3,2,k}+x_{3,1,3} = 1, \; \forall \; k \\
\end {aligned}
\end {split}
$$
...
Sudoku_model_specific <- Sudoku_model %>% 
  add_constraint(Vx[1, 1, 1] == 1) %>%  # Set the cell in top left to the number 1
  add_constraint(Vx[1, 2, 2] == 1) %>%  # Set the cell in row 1, column 2 to the number 2
  add_constraint(Vx[1, 3, 3] == 1) %>%  # Set the cell in row 1, column 3 to the number 3
  add_constraint(Vx[2, 1, 4] == 1) %>%  # Set the cell in row 2, column 1 to the number 4
  add_constraint(Vx[2, 2, 5] == 1) %>% 
  add_constraint(Vx[2, 3, 6] == 1) %>% 
  add_constraint(Vx[3, 1, 7] == 1) %>% 
  add_constraint(Vx[3, 2, 8] == 1) %>% 
  add_constraint(Vx[3, 3, 9] == 1)

Sudoku_result_specific <- solve_model(Sudoku_model_specific, with_ROI(solver = "glpk", verbose = TRUE))
