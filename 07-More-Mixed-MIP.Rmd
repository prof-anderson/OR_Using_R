...
result_symphony <-  solve_model(model, 
...
result_lpsolve <-  solve_model(model, 
...
Many of these packages are being used directly in other packages. While we can't see which packages are being used most often for `ompr`, we can see which ROI plugins are downloaded most often which is probably a good proxy.    
\vspace{12pt}
