...
$z_{t,B}$ is the inventory of Ants at the end of week _t_.

A critical concept to understand is the inventory balance.  Let's demonstrated it with Ants and arbitarily pick week 3.  We know that the ending inventory for week 3, $z_{3,A}$ is equal to the inventory from the from the previous week, $z_{2,A}$, plus amount produced in week 3, $x_{3,A}$, minus the amount sold (in our case demand), $D_{3,A}$.  

$$z_{3,A}=z_{2,A} + x_{3,A} - D_{3,A}$$

The inventory balance constraints follow a similar form for each week other than week 1.  For week 1, the previous inventory is fixed rather than a variable and can be framed as the following.
...
```{r, eval=TRUE}
promod <- add_constraint (promod, VzB[tt-1] + VxA[tt] - 
...
```{r, eval=TRUE}
promod <- add_constraint (promod, VzA[tt] + VzB[tt] <= maxinv, 
                          tt = 1:nTime)
