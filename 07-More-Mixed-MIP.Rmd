...
library(bookdown); library(rmarkdown); rmarkdown::render("07-More-Mixed-MIP.Rmd", "pdf_document")
```

```{r Ch7setup, echo=FALSE, warning=FALSE, message=FALSE}
...
To enforce this constraint, we can simply add the $x$ variables for customer 1 for each of the warehouses.
...
Now that we have prepared the model, we are now ready to work on solving the model. We will start with using `glpk` as we have done in previous examples.\
...
One of the big benefits of `ompr` is that it separates the process of formulating the optimization model from that of solving it and thereby lets us easily switch to other solvers.
...
First, `ompr` status indicates an `error`.  Just as we saw in Chapter 2's unbounded case, `ompr` interprets the status code from the solver by way of `ROI` as not being solved to optimality for *some* reason such as being told to terminate after the first feasible solution was found, time limit reached, node limit reached, the MIP was infeasible, the MIP was unbounded, or some other issue.

A second thing to note is that in my randomly generated instance, the very first feasible solution it found, happens to have the same objective function value as the optimal solution found to optimality earlier by `glpk` and `symphony`.
...
Let's revisit our drone manufacturing example but accounting for time.  In this case, we'll look at producing Ants and Bats over time while accounting for varying weekly demand, inventory carrying costs, setup costs, marginal production costs, and  beginning inventory.  The planning horizon is four weeks.  
...
- $x_{t,A}$ is the number of Ants to make in week _t_.
- $x_{t,B}$ is the number of Bats to make in week _t_.
...
Our goal is to minimize the total cost.  Let's look at the total cost with respect to Ants.  It is a combination production cost, setup costs, and carrying costs.

$$
\begin{split}
\begin{aligned}
\text{} \; & \sum\limits_{t=1}^{4}C^P_A\cdot x_{t,A} + C^S_A\cdot y_{t,A} + C^I_A\cdot z_{t,A}\\
\end{aligned}
\end{split}
$$
...
\text{min   } \; & \sum\limits_{t=1}^{4} ( C^P_A\cdot x_{t,A} + C^S_A\cdot y_{t,A} + C^I_A\cdot z_{t,A}+\\
                 & C^P_B\cdot x_{t,B} + C^S_B\cdot y_{t,B} + C^I_B\cdot z_{t,B} ) \\
\text{s.t.: } \; & z_{1,A}=B^I_{A} + x_{1,A} - D_{1,A}\\
                 & z_{1,B}=B^I_{B} + x_{1,B} - D_{1,B}\\
                 & z_{t,A}=z_{t-1,A} + x_{t,A} - D_{t,A}, \; \forall t>1\\
                 ...
                 & x_{t,A} \leq L^P \cdot y_{t,A} \; \forall t\\
                 & x_{t,B} \leq L^P \cdot y_{t,B} \; \forall t\\
                 & x_{t,A} \; x_{t,B}, \; z_{t,A} \; z_{t,B} \geq 0, \; \forall \; t \\
...
Now we need to create our linking constraint for each product to connect our production amount variable and the decision to produce that product.
We will use the Big M method and the maximum production value as our Big M value.
...
Now let's put a limit on our joint inventory of products.
...
Now let's put a limit on our joint production capacity of products.

```{r, eval=TRUE}
promod <- add_constraint (promod, VxA[tt] + VxB[tt] <= lmaxpro, 
                          tt = 1:nTime)
```
