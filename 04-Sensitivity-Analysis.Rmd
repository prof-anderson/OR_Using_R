...
pander(rbind(base_case_res, inc_mc_res, inc_wd_res),
       caption="Production Plan with 10,000 More Wood")
```
...
```{r Shadow_Price_Single_Chair}
chair_res_used<-cbind(rduals1,c(2,8,6,40,0))
colnames(chair_res_used)<-c("Row Duals", "Resources Used")
pander(chair_res_used, 
       caption="Resources Used by a Chair and their Shadow Prices")
```
...
```{r Shadow_Price_Desks}
desk_res_used<-cbind(rduals1,c(2,6,4,25,0))
colnames(desk_res_used)<-c("Row Duals", "Resources Used")
pander(desk_res_used, 
       caption="Resources Used by a Desk and their Shadow Prices")
```
...
```{r Shadow_Price_Table}
table_res_used<-cbind(rduals1,c(4,4,8,25,0))
colnames(table_res_used)<-c("Row Duals", "Resources Used")
pander(table_res_used, 
       caption="Resources Used by a Table and their Shadow Prices")
```
...
## Exercise: Revisiting Transportation
