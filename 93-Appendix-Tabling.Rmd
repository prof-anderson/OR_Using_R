...
Since `col.names` works well, there is no reason to spend more time on it.  Let's instead show how to replace row names for a table. The following example gives bad row names to a matrix that I want to replace and use with an better set of row names.  It then generates three tables showing three situations.  
...
kbl (m, booktabs=T, caption="Retain Row Names by Using `row.name=T`",
     ...
     row.names=T) |>   
  kable_styling (latex_options = "hold_position") |>
  footnote (general="Not the intended result because it has bad row names.")

kbl (m, booktabs=T, caption="Drop Row Name by Using `row.name=F`",
     col.names=c("C1", "C2"), 
     row.names=F) |>
  kable_styling (latex_options = "hold_position") |> 
  footnote (general="Not ideal but at least it avoids the bad row names.")
...
kbl (cbind(as.matrix(GR),m), booktabs=T,
     ...
     row.names=F) |>
  kable_styling (latex_options = "hold_position") |> 
  footnote (general="Good solution.")
...
kbl (cbind(as.matrix(GR),m),
     caption="Default format using kbl without booktabs.",
     col.names=c("", "C1", "C2"), 
     row.names=F) |>
  kable_styling(latex_options = "hold_position")

kbl (cbind(as.matrix(GR),m), booktabs=T,
     caption="Kable (kbl) using booktabs.",
     col.names=c("", "C1", "C2"), 
     row.names=F)|>
  kable_styling(latex_options = "hold_position")
```
...
```{r}
NProd <- 52
mbig <- matrix(1:(2*NProd), ncol= NProd)
