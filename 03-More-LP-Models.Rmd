...
Let's implement a four product model requiring varying levels of five limited resources.  The profit per product is described in Table 3.1 and and the resource consumption per product is given in table 3.2.  We will start our implementatoin by creating the appropriate data structures in `R`.
...
Let's show how to set up a basic implementation of the tranportation problem in `ompr`. This demonstrates the use of double subscripted variables. Also, it should be customized for a particular application of being demand or supply constrained but it will give a good running head start.
\vspace{12pt}
...
Cost <- matrix (c(65, 36, 65, 57, 30, 70, 22, 30, 55, 42, 60, 42),
                nrow = NSupply,
...
```{r Show_Transp_Cost, fig.margin=TRUE}
grid.table (Transp)
```
...
res.transp <-solve_model(transportationmodel, 
                                    with_ROI(solver ="glpk"))
...
A common characteristic of this kind of problem is then that the inflow at each distribution center must equal the outflow.  This means that for Minnesota, $(i=1)$, we would would have a relationship of the following.

$$x_{1,1}+x_{2,1}=y_{1,1}+y_{1,2}+y_{1,3}+y_{1,4}$$ 

This lends itself to a summation.

$$\sum_{i=1}^2 x_{i,1}=\sum_{i=k}^4 y_{1,k}$$

We can then generalize this for every distribution center, _j_.

$$\sum_{i=1}^2 x_{i,j}=\sum_{i=k}^4 y_{j,k} \; \forall \; j$$
...
Let's demonstrate this by extracting data from a model, `model1` that we have previously built in `ompr`.
...
# min cx; Ax<=B
library (Rglpk)
constraints <- extract_constraints(model1)
A_matrix <- constraints$matrix
B_vector<- extract_constraints(model1)$rhs
CC <- objective_function(model1)
C_vector <- CC$solution
direction <- constraints$sense

A_matrix  # Matrix of constraint coefficients
B_vector  # Constraint right hand sides
C_vector  # Objective function coefficients

res <- Rglpk_solve_LP(C_vector, A_matrix, direction, B_vector, 
                      max=TRUE)

```

Let's review results of solving the `Rglpk` solve version of the model. 

```{r}
res$status
```
The status returned `Rglpk` is zero.  This may sound bad but the package's help file indicate that returning a status value of zero means that it found an optimal solution.

Let's look at the objective function value 
