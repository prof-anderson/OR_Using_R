...
```{r Examine-Results-of-Changed-Model, fig.margin=TRUE}
results.products <- matrix (rep(-1.0,NProd), 
                            nrow = NProd, 
                            ncol=1,
                            dimnames=c(ProdNames,c("x")))
...
$$0.6 \cdot Ants-0.4 \cdot Bats+0.6 \cdot Cats \leq 0$$
...
kbl(comparative, booktabs=T, 
    caption ="Compare Baseline and Production Plan with Blending Constraint")|>
  kable_styling(latex_options = "hold_position")
```
...
```{r Display-Trail-Mix-Data, fig.margin=TRUE}
...
```{r, nurse-scheduling, fig.margin=TRUE}
