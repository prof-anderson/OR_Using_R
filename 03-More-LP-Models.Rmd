...
kbl(Available, booktabs=T, caption="Amount of Each Resource Available") |>
  kable_styling(latex_options = "hold_position")
```
...
kbl(Combined, booktabs=T, caption="Resources Used by Each Product and Available") |>
  kable_styling(latex_options = "hold_position")
```
...
Effectively, we are saying that $\frac{Ants}{Ants+Bats+Cats} \leq 0.40$ 

Alas, this is not a linear function so we need to clear the denominator.

$$Ants \leq 0.40 \cdot (Ants+Bats+Cats)$$ 

We like to get all the variables on the left side so let's move them over.

$$Ants - 0.4 \cdot Ants-0.4 \cdot Bats-0.4 \cdot Cats \leq 0$$ 
...
```{r Compare-Blending-to-Base-Results, fig.margin=TRUE,fig.width=3}
...
```{r Fig-Transportation, echo=FALSE, out.width="2in", fig.show='hold',fig.align='center', fig.cap="Transporation Example"}
...
```{r Fig-Transshipment, echo=FALSE, out.width="2in", fig.show='hold', fig.align='center', fig.cap="Transshipment Application."}
...
```{r Fig-ompr-structure, echo=FALSE, fig.show='hold', fig.align='center', out.height="1in", fig.cap="Relationship between ompr and solvers."}
...
```{r Fig-ompr-to-glpk, echo=FALSE, out.width="1.5in", fig.cap="Relationship between ompr and Rglpk."}
