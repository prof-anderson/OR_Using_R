...
Similarly, we can display the resources used by each product and the amount of each resource available. See both tables on the right. Table 3.2 displayes the resources used by each product and Table 3.3 the amount of resources available from each product.   
...
prodmodel <- MIPModel() %>%
  ...
  add_constraint (sum_expr(Resources[j,i]*x[i], i=1:NProd) 
...
$$
 ...
 \begin{aligned}
    \text{Max:} & \\
                & Profit=20*Chairs+10*Desks+16*Tables \\
    \text{S.t.} & \\
...
Convert our generalized production planning LP, displayed on the right, into standard form.  
...
```
```{marginfigure}
**Table 3.14**: Generalized LP case
```