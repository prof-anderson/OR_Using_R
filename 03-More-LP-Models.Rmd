...
We could also reframe the model more algebraically. Let's use subscripts to differentiate between products and resources. We can define to  *i=1* to represent Ants, *i=2* to represent Bats, and *i=3* to represent Cats.  Similarly, *j=1* represents machining, *j=2*, represents assembly, etc.
...
```{r Display-Resources, tidy=FALSE, echo = FALSE, results='asis'}
...
```{r Display-Available, tidy=FALSE, echo = FALSE, results='asis'}
...
The constraint is simply  $\frac{Cats}{Ants+Bats+Cats} \leq 0.40$.  Alas, this is not a linear function since we are dividing a variable by a function of variables so we need to clear the denominator.  For demonstration purposes, we will write out each step.

$$Cats \leq 0.40 \cdot (Ants+Bats+Cats)$$ 
...
trailmixmodel <- MIPModel()                             |>
 ...
 add_constraint(sum_expr(MixChar[j,i]*Vx[i],i=1:NMix)  # LHS of constraint
...
$$
 ...
 \begin{aligned}
    ...
    \text{s.t. } & \sum_{i} x_{i,j} \leq D_j \; \forall \; j\\
...
$$
 ...
 \begin{aligned}
    ...
    \text{s.t.: } & \sum_{j} y_{j,k} \leq D_k \; \forall \; k\\
                       & \sum_{j} x_{i,j} \leq S_i \; \forall \; i\\
...
$$
 ...
 \begin{aligned}
    ...
    \text{s.t.:  } & B_{p} + x_{p,1} - D_{p,1} = y_{p,1}   \; \forall \; p,  \\
...
Any linear program with inequality constraints can be converted into what is referred to as standard form. \index{Standard form}
...
StdModel <- MIPModel() |>
 # To avoid name space conflicts, using a prefix of V for ompr variables.
...
The actual solver used such as `glpk,` `symphony,` or `lpSolve` take the linear program in a different form though and `ompr` processes it into a form they can handle. \index{glpk} \index{symphony} \index{lpSolve} \index{ROI}
