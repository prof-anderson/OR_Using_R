...
We can define that *i=1* represents Chairs, *i=2* represents Desks, and *i=3* represents Tables.
...
Similarly, *j=1* represents fabrication, *j=2*, represents assembly, etc.
...
We can then rewrite the above formulation as the following.
...
Also, another good practice is to use a mnemonic to help suggest the meaning of data.
...
(Some people will swap this around and use capital letters for variables and lower case letters for data - it doesn't really matter as long as a model is consistent.)\
\vspace{12pt}
...
Let's change the time required for painting a chair to 20.
...
In our example, we might say that Chairs can make up no more than 40% of the overall product plan.
...
Let's add the constraint that chairs can't make up more than 40% of the total production.
\vspace{12pt}

Effectively, we are saying that $\frac{Chairs}{Chairs+Desks+Tables} \leq 0.40$ \vspace{12pt}

Alas, this is not a linear function so we need to clear the denominator.
\vspace{12pt}

$Chairs \leq 0.40 \cdot (Chairs+Desks+Tables)$ \vspace{12pt}

We like to get all the variables on the left side so let's move them over.
\vspace{12pt}

$Chairs - 0.4 \cdot Chairs-0.4 \cdot Desks-0.4 \cdot Tables \leq 0$ \vspace{12pt}
...
This naturally lends itself to potential network diagrams.
In this case, $x_{i,j}$ is the amount of product to ship from node i to node j.
...
In this formulation we need to make sure that we don't ship out more than capacity from each supply node.
...
Let's show how to set up a basic implementation of the tranportation problem in ompr.
This demonstrates the use of double subscripted variables.
Note that I have not defined the data.
...
Any linear program in inequality constraints can be converted into what is referred to as standard form.
...
It makes little difference as to whether the objective function is a *min* or a *max* function since a min objective function can be converted to a max objective function by multiplying everything a negative one.
The converse is also true.
\vspace{12pt}

Lastly is a step where all of the inequalities are replaced by strict equality relations.
...
The actual algorithm for the Simplex algorithm is beyond the scope of what we will cover this in book.\
\vspace{12pt}
...
The result is that any linear program can be thought of in a form such as the following.
...
Let's demonstrate this by extracting data from a model, `Base3VarModel` that we have previously built in `ompr`.
Simply typing in the name of the unsolved model will give a summary of the model characteristics.
This was a problem with three variable and five constraints.\
\vspace{12pt}
...
When we build the model in `ompr`, it builds a representation of *A, B, C,* and *x*.
Now, let's examine the the data structures inside the model.
...
Notice this gives us the *A* matrix (aptly named `matrix` by `ompr`).
The set of right hand side values of the constraints (the *B* vector) is named `rhs` by `ompr`.
Lastly, the equality or type of inequality constraint is stored in a vector named `sense`.
\vspace{12pt}

The benefit of `ompr` and algebraic modeling more generally is highlighted as models get more complex with multiple subscripted variables (especially double or triple subscripted variables), and lots of constraints.
Creating the Operations Research analyst could be build the *A* matrix on their own, keeping track of which row corresponds to a particular variable becomes very difficult.
For an example of how this can be done, see the *DEA Using R* book's chapter on a matrix implementation of DEA.
\vspace{12pt}
