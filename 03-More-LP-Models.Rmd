...
```{r Show_Transp_Cost, eval=FALSE, fig.margin=TRUE}
grid.table (Cost)
```
...
```{r Fig-ompr_structure, echo=FALSE, fig.margin=TRUE, out.width="2in"}
...
Earlier in the chapter, we created data structures for the generalized version of the optimization model. Recall that we `Resources` as a matrix of the resources used by each product - this corresponds to our $A$ matrix.  We also created a single column  matrix, `Available` for the amount of each resource available which is our $B$
vector.  The `Profit` single row matrix serves as our $C$ vector.  The last item that we need to do is to specify a direction for each inequality. 
\vspace{12pt}
...
```{r Fig-ompr_to_glpk, echo=FALSE, fig.margin=TRUE, out.width="2in"}
...
res2 <- Rglpk_solve_LP(obj=Profit,    # C vector
                       ...
                       Available,     # B vector
                       max=TRUE)      # False does a minimization
