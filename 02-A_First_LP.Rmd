...
We will now extend the previous model to account for a third product, the Cat robot. The goal is now to find the best product mix of Ants, Bats, and Cats. See the table to the right with a summary of the new situation.
...
Let's assume that the sales manager comes in and says that we have a contractual requirement to deliver 300 Ants to customers. \vspace{12pt}
...
Note that the constraint on the minimum number of bats could also be implemented by changing the lower bound on the bats variable to be 300 instead of zero. Also, the upper limit on Cats could be set as an upper bound on the Cats variable.
