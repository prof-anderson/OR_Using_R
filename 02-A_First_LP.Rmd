...
```
```{marginfigure}
Formula 1: Optimization mode. Base equation with 2 variables
```
...
```
```{marginfigure}
Formula 2: Optimization model adding a third product(variable)
```
...
```
```{marginfigure}
Formula 3: Optimization model for a Non Feasible solution 
```
...
```
```{marginfigure}
Formula 4: Optimization model for Multiple Optima solution 
```
...
pander(res2b,
caption="Results 2b - A Second of Multiple Optima")
```
...
pander(rbind(res2a,res2b), 
caption="Comparison of Two Alternate Optimal Solutions")
```
...
```
```{marginfigure}
Formula 5: Optimization model with a dedundant constraint 
```
...
```
```{marginfigure}
Formula 6: Optimization model with an unbounded solution 
```
...
```
```{marginfigure}
Formula 7: Optimization model for Graphically Solving LPs with R exercise 
```
...
$$
```{marginfigure}
Formula 8: Update formalation for constraint 1. 
```
...
$$
```{marginfigure}
Formula 9: Update formalation for constraint 2. 
```
...
# create the series
x1 = seq(0,200)
pander(head(as.matrix(x1)), 
       caption="Beginning of Sequence x1")
```
...
pander(head(seriesDf),
       caption="Some datapoints from the lines for the constraints")
```
...
```
```{marginfigure}
Formula 10: More complex formulation
```
...
```
```{marginfigure}
Figure2. especial para esplicar algo
```
