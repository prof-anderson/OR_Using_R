...
\[@hillier2020a\]\[@winston2003\]\[@baker2015\]\[@tovey\]\[@IsmayIntroductionStatisticalData\]\[@AdlerNutshell2012\]
...
```{r Fig-First-Constraint, echo=FALSE, fig.margin=TRUE, out.width="3in",  fig.align='center', fig.cap="Drawing a First Constraint."}
library(knitr)
...
```{r Fig-Second-Constraint, echo=FALSE, fig.margin=TRUE, out.width="3in",  fig.align='center',fig.cap="Adding the Assembly Constraint."}
...
```{r Fig-Third-Constraint, echo=FALSE, fig.margin=TRUE, out.width="3in",  fig.align='center', fig.cap="Adding the Testing Constraint." }
...
The first line initializes an empty model and stores it in `model0`. We can see that the model is empty by simfply displaying the summary of `model0`.\
...
```{r Base-2Var-Solution, fig.margin = T}
kbl(res0, booktabs=T,
...
```{marginfigure}
**The pipe symbol**: Represented by `|>` will let us do this more compactly and efficiently. It takes a little getting used to but is a better way to build the model.  Also, note that the piping operator requires `R` version 4.1.0. or higher.
```
