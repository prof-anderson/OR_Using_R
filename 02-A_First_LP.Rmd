...
```{r infeasible_case}
model1infeas <- 
  add_constraint(model1, Chairs >= 300)  #THIS IS THE NEW CHANGE
  
result1infeas <-  solve_model(model1infeas, with_ROI(solver = "glpk"))

```

In this case, we are going to simply add the new constraint to `model1` and create a new model to solve.
...
model2a <- set_objective(model1, 20*Chairs - 10*Desks - 16*Tables, "max") 
...
pander(res2a,caption="Results 2a - A First of Multiple Optima")
...
```{r Show_Second_Solution_from_Multiple_Optima}
...
pander(res2b,caption="Results 2b - A Second of Multiple Optima")
...
```{r Summarize_Results_of_Multiple_Optima}
...
#### Formulation of an Unbounded Situation
...
#### Implementation with Unbounded Modification
