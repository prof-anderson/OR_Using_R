...
In the base case, our objective function is to Maximize Profit. We can't express it though until we define our decision variables.

It is good practice to very clearly and precisely define  decision variables.  In this case it is straightforward but they can get much more complicated as we move into richer and larger models.
...
This is how we would do it without a piping operation.

```{r base_case_no_pipes}
model0  <- MIPModel()     # Initialize an empty model!

model0a <- add_variable(model0, Chairs, type = "continuous", lb = 0) 
model0b <- add_variable(model0a, Desks, type = "continuous",lb = 0)

model0c <- set_objective(model0b, 20*Chairs + 14*Desks, "max")

model0d <- add_constraint(model0c, 6*Chairs + 2*Desks <= 2000) #fabrication
model0e <- add_constraint(model0d, 8*Chairs + 6*Desks <= 2000)  #assembly
model0f <- add_constraint(model0e, 6*Chairs + 4*Desks <= 1440)  #machining
model0g <- add_constraint(model0f, 40*Chairs + 25*Desks <= 9600) #wood
...
There are no constraints or variables.  The next line takes `model0` and adds a variable to it, creating an enhanced `model0a`.  Again, we can confirm that our model is gradually getting built up by displaying the new model.
...
The first line initializes an emtpy model and the next three lines are adding variables.  These variables are continuous (as compared to integer or binary), and non-negative. The variables are made non-negative by setting a zero lowerbound (`lb=0`).  The lowerbound can be set to other values such as a minimum production level of ten.  Also upperbounds for variables can be set using `ub` as a parameter. 
...
Lastly, we tell it to solver our model.  We will select a particular solver engine, `glpk` but other options exist.

This process built up the model line by line incrementally taking the previous model, adding a new element, and storing it in a new model.  Note that we could keep placing them into the original model if we had wished such as the following.

```{r base_case_no_pipes_single_model}
model0  <- MIPModel()     # Initialize an empty model!

model0 <- add_variable(model0, Chairs, type = "continuous", lb = 0) 
model0 <- add_variable(model0, Desks, type = "continuous",lb = 0)

model0 <- set_objective(model0, 20*Chairs + 14*Desks, "max")

model0 <- add_constraint(model0, 6*Chairs + 2*Desks <= 2000) #fabrication
model0 <- add_constraint(model0, 8*Chairs + 6*Desks <= 2000)  #assembly
model0 <- add_constraint(model0, 6*Chairs + 4*Desks <= 1440)  #machining
model0 <- add_constraint(model0, 40*Chairs + 25*Desks <= 9600) #wood
...
This is equivalent to doing something such as the following where we assign the value of _2_ to _a_.  We then multiple _a_ by _3_ and assign the result again to _a_.    
...
Note that this is part of why `<-` as an assignment operator is better than an equals sign, `=` which would make the reader think that if _a=3*a_ then _a_ must be zero. 
...
```{r construct_constraints_for_display}
constr0<-extract_constraints(model0)    # Pass the constraints out of  model
constr00<- (cbind(as.matrix(constr0$matrix), # Get the matrix of data
                  as.matrix(constr0$sense),  # Get the inequality 
                  as.matrix(constr0$rhs)))   # Get the Right Hand Side

dimnames(constr00)<-c(list(c("Fabrication", "Assembly","Machining", "Wood")),
                      list(c("Chairs", "Desks","Relationship","RHS")))

pander(constr00, caption="Elements of the Constraints")
```
...
The pipe symbol will let us do this more compactly and efficiently. It takes a little getting used to but is a better way to build the model.  

Here is the equivalent process using a piping operator without all the intermediate partial models being built.  Notice how it is a lot more concise.  We will typically use this approach for model building but both approaches are functionally equivalent.  
...
Note that we load a pair of additional packages.  The magrittr package gives us a piping function that simplifies model building.  The dplyr package is like a Swiss army knife for a providing a broad range of different data process tools. 
...
For the sake of simplicity, we will again implement our R formulation using explicit variables and data consistent with the first formulation.  
...
This is interesting - even though we added a new product to our product portfolio, production decision stayed the same.  
...
Note that the constraint on the minimum number of chairs could also be implmented by changing the lower bound on the chairs variable to be 300 instead of zero.  Also, the maximum limit on Tables could be set as an upper bound on the Tables variable.
...
Notice that since the solver status was infeasible, the  values for the decision variables are not feasible and therefore cannot be considered a reliable (or possible) production plan.  This highlights why the solver's status should always be tested before results are discussed.
...
model2a <- MIPModel() %>%
  ...
  set_objective(20*Chairs - 10*Desks - 16*Tables, "max") %>%
...
pander(res2a,caption="Results 2a - Drawn from Multiple Optima")
...
pander(res2b,caption="Results 2b - Drawn from Multiple Optima")
...
```{r}
pander(rbind(res2a,res2b), caption="Comparison of Two Alternate Optimal Solutions")
```
...
Again, the solver status is report that the problem is *infeasible* rather than *unbounded* but by inspection, the LP is feasible.  A value for Tables = 1000, Chairs=Desks=0, satisfies all of the constraints.  Perhaps I am missing something in my model or it might be a bug in one of the packages that is misreporting the solver status.  I need to do more digging to check on that.
...
```
## Exercise 2.1
This is a great and wonderful problem...  

```{r}
# this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good.... this is really good....
...
#really good.... this is really good.... this is really 
#good.... this is really good.... this is really good.... 
#this is really good.... this is really good.... this is 
...
$$
 ...
 \begin{aligned}
    \text{Max  }   & 150*x+200*y \\
    \text{subject to } & 40*x+60*y \leq 7400 \\
                       & 20*x+25*y \leq 3300 \\
                       & x, y \geq 0  
  \end{aligned}
  \end{split}
...
To graph a line in R you need to have a series of coordinates, you can't just input a formula and see it on a chart. Here are some steps to help turn your contraints into data serieses.
...
pander(head(as.matrix(x1)),caption="Beginning of Sequence x1")
```
...
# 2 constraints in this equation so there are 2 y variables, y1 and y2.  
# If you have more constraints you can just keep adding them here.
seriesDf = data.frame(x1, y1=r1y(x1), y2=r2y(x1))

# take a look at the generated data
pander(head(seriesDf),caption="Some datapoints from the lines for the constraints")
...
```{r}
# this function returns a data frame with the intersection point of 2 lines
# pass in the model for each line 
lmInt <- function(line1, line2) {
  ...
  data.frame(x=round(x, 2), y=round(y, 2)) #create a data frame with x and y
}
...
intersection <- lmInt(lm(seriesDf$y1~seriesDf$x1), lm(seriesDf$y2~seriesDf$x1))

head(intersection)

```

At this point we have everything we need to see our results on a graph. There are a few ways to build graphs in R but one very popular and powerful library is ggplot. ggplot was written by [Hadley Wickham](http://hadley.nz/) to implement the [_Grammar of Graphics_](https://www.amazon.com/Grammar-Graphics-Statistics-Computing/dp/0387245448). That book is kind of expensive, a more affordable intro would be [Hadley's overview](http://vita.had.co.nz/papers/layered-grammar.pdf).
...
# first transate the constraints:
...
dataFrameTwo = data.frame(x2, y1=constraint1(x2), 
                          y2=constraint2(x2) , y3=constraint3(x2))

# because constraints are mixed leq and geq, we take the min of y2 and y3

dataFrameTwo <-  transform(dataFrameTwo, z = pmax(y1,pmin(y2,y3)))

# get the intersections of the lines
intersection1 <- 
  lmInt(lm(dataFrameTwo$y3~dataFrameTwo$x2), lm(dataFrameTwo$y2~dataFrameTwo$x2))
intersection2 <- 
  lmInt(lm(dataFrameTwo$y3~dataFrameTwo$x2), lm(dataFrameTwo$y1~dataFrameTwo$x2))

#plot it
ggplot(dataFrameTwo, aes(x = x2)) + 
  geom_line(aes(y = y1), color = 'aquamarine', size = 2) + #you can change line size
  geom_line(aes(y = y2), color = '#99ffbb', size = 2) + #you can use hex colors
  geom_line(aes(y = y3), color = 'white', size = 2) +
  geom_ribbon(aes(ymin=y1,ymax = z), fill = 'gray80') + #you can change alpha on colors
  geom_point(x = intersection1$x, y = intersection1$y, color = "green", size = 5) +
  geom_point(x = intersection2$x, y = intersection2$y, color = "red") 
...
Note that we only used 2 variables, so we were able to see this in a 2 dimentional space. If you add another variable to equation you will have to map this in a 3 dimentional chart which you may want to try a different charting library for like [plotly](https://plot.ly/r/). If you have more variables than that using the graphical method will become very hard!
